...
export default function Click() {
  ...
  const { isGettingClicks } = useSelector((state) => state.link);
  ...
  const handleSubmit = async (e) => {
    ...
    const shortCode = formData.customAlias;
    await dispatch(getMyClicksThunk({data: shortCode, page: 1, limit: 3})).unwrap();
    setErrors({});
    setFormData({
      customAlias: "",
    });
  ...
  return (
    <div className="flex items-center justify-center h-full pb-25">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Custom Alias{" "}
                <span
                  className={`${errors.customAlias ? "text-red-500" : ""} font-normal`}
                >
                  *
                </span>
              </label>
