...
export const analytics = async (req, res) => {
  try {
    ...
    // Only generate insights if there are clicks
    if (allClicks.length > 0) {
      ...
      if (topDevice) {
        ...
        const deviceInsight = `${topDevice[0]} users account for ${devicePercentage}% of traffic.`;
      ...
      if (topCountryEntry) {
        const topCountry = `The most visited country is ${topCountryEntry[0]}`;
      ...
      if (topReferrerEntry) {
        const topReferrer = `The most visited referrer is ${topReferrerEntry[0]}`;
      ...
      // Find peak hour
      const peakHourEntry = Object.entries(hourlyTraffic).sort(
        (a, b) => b[1] - a[1]
      )[0];
